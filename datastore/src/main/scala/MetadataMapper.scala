...
object MetadataMapper {
  ...
  private def sumComponents(components: List[BigDecimal]): BigDecimal = {
    if (components.head >= 0) components.sum else components.reduceLeft(_ - _)
  }

  private def convertAllComponentsToDegrees(components: List[BigDecimal]): List[BigDecimal] = {
    components.zipWithIndex.map { case (component: BigDecimal, index: Int) => component / math.pow(60, index) }
  }

  private def convertToDMSComponents(coordinate: String): List[BigDecimal] = {
    coordinate.split(Array(Degrees, Minutes, Seconds)).map(s => BigDecimal(s.trim)).take(3).toList
  }
}
...
class MetadataMapper(private val genericMediaItemDataMapping: ComplexObject, private val dataMapping: Map[String, ComplexObject]) {
    ...
    val combinedDataMapping = dataMapping.get(fileFormat).map(mappingForMediaType => {
      ...
      ComplexObject(mappingForMediaType.objectType,
        mappingForMediaType.children ++ genericMediaItemDataMapping.children.flatMap {
          case mapping@(_, objectDefinition) => if (!mappingForMediaType.children.values.toList.contains(objectDefinition)) Some(mapping) else None
        })
    }).getOrElse(genericMediaItemDataMapping)

    import scala.collection.JavaConversions._
    new LinkedHashModel(convertMetadataToRDF(valueFactory.createIRI(uri), combinedDataMapping, metadata + (HashPredicateKey -> hash) + (FileFormatPredicateKey -> fileFormat)))
  }
  ...
  private def simpleObjectAsLiteral(dataType: DataType.Value, properties: List[String], metadata: Map[String, String]): Option[Literal] = {
    dataType match {
