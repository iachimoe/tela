...
object SmackXMPPSession {
  ...
  private def createTCPConnectionToServer(xmppSettings: XMPPSettings): XMPPTCPConnection = {
    ...
    val config = XMPPTCPConnectionConfiguration.builder().
      ...
      setSecurityMode(xmppSettings.securityMode match {
        ...
        case invalidMode => throw new IllegalArgumentException("Invalid security mode " + invalidMode)
      }).build()
  ...
  private class ContactListChangeListener(private val xmppSession: SmackXMPPSession) extends RosterListener {
    override def entriesAdded(addresses: java.util.Collection[Jid]): Unit = {
      ...
      xmppSession.sessionListener.contactsAdded(addresses.map(address => ContactInfo(address.asUnescapedString(), xmppSession.getPresenceForContact(address.asUnescapedString()))).toList)
    }
...
private[xmpp] class SmackXMPPSession(private val connection: XMPPConnection, private val sessionListener: XMPPSessionListener, private val xmppSettings: XMPPSettings) extends XMPPSession {
  ...
  override def getContactList(): Unit = {
    ...
    import scala.collection.JavaConversions._
    sessionListener.contactsAdded(Roster.getInstanceFor(connection).getEntries.map(
      entry => ContactInfo(entry.getJid.asUnescapedString(), getTelaPresenceFromXMPPPresence(Roster.getInstanceFor(connection).getPresence(entry.getJid)))).toArray.toList)
  }
  ...
  override def getPublishedData(user: String, nodeName: String): String = {
    ...
    getNode(manager, nodeName, user).flatMap(node => {
      import scala.collection.JavaConversions._
      node.getItems[PayloadItem[SimplePayload]]().toList.headOption.map(_.getPayload.toXML.toString)
  ...
  override def sendCallSignal(user: String, data: String): Unit = {
    ...
    signal.setTo(JidCreate.fullFrom(if (XmppStringUtils.isFullJID(user)) user else user + "/" + DefaultResourceName))
