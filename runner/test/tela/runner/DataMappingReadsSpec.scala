...
import tela.runner.DataMappingReads.*
...
class DataMappingReadsSpec extends BaseSpec {
  "DataMappingReads" should "deserialize JSON into a ComplexObject" in {
    val expected = ComplexObject(new URI("http://schema.org/AudioObject"), Map(
      ...
      new URI("http://schema.org/author") -> ComplexObject(new URI("http://schema.org/Person"),
        Map(new URI("http://schema.org/name") -> SimpleObject(Vector("xmpDM:artist"), multiplicityStrategy = Split)))))

    val song = Json.obj(DataStoreConnection.ObjectTypeKey -> "http://schema.org/AudioObject",
      "children" -> Json.obj("http://schema.org/genre" -> Json.obj("properties" -> Json.arr("xmpDM:genre"), "dataType" -> "Text"),
        ...
        "http://schema.org/author" -> Json.obj(DataStoreConnection.ObjectTypeKey -> "http://schema.org/Person",
          "children" -> Json.obj("http://schema.org/name" ->
            Json.obj("properties" -> Json.arr("xmpDM:artist"), "dataType" -> "Text", "multiplicityStrategy" -> "Split")))))
